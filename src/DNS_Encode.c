...
int DNS_to_bytes(unsigned char* bytes, DNS_PACKET dns_packet)
{
    ...
    bytes[24+offset] = ((dns_packet.answer->rdata    >>  0) & 0xFF);
...
void bytes_to_DNS(DNS_PACKET dns_packet, unsigned char* bytes)
{
    ...
    // header
    dns_packet.header->id 				  =	((bytes[ 0+offset] << 8) & 0xFF00     )| 
    										((bytes[ 1+offset] << 0) & 0xFF  	  );
    dns_packet.header->qr 				  =	((bytes[ 2+offset] >> 7) & 0x01		  );
    dns_packet.header->opcode 			  =	((bytes[ 2+offset] >> 3) & 0x80 	  );
    dns_packet.header->aa 				  =	((bytes[ 2+offset] >> 2) & 0x01		  );
    dns_packet.header->tc 				  =	((bytes[ 2+offset] >> 1) & 0x01		  );
    dns_packet.header->rd 				  = ((bytes[ 2+offset] >> 0) & 0x01		  );
    dns_packet.header->ra 				  =	((bytes[ 3+offset] >> 7) & 0x01 	  );
    dns_packet.header->z				  =	((bytes[ 3+offset] >> 4) & 0x08 	  );
    dns_packet.header->rcode			  =	((bytes[ 3+offset] >> 0) & 0x0F 	  );
    dns_packet.header->qdcount			  =	((bytes[ 4+offset] << 8) & 0xFF00 	  )| 
    										((bytes[ 5+offset] << 0) & 0xFF 	  );
    dns_packet.header->qdcount			  = ((bytes[ 6+offset] << 8) & 0xFF00 	  )| 
    										((bytes[ 7+offset] << 0) & 0xFF 	  );
    dns_packet.header->ancount			  = ((bytes[ 8+offset] << 8) & 0xFF00 	  )| 
    										((bytes[ 9+offset] << 0) & 0xFF 	  );
    dns_packet.header->arcount			  = ((bytes[10+offset] << 8) & 0xFF00 	  )| 
    										((bytes[11+offset] << 0) & 0xFF  	  );
    
    // question
    while (bytes[12+offset] != '\0')
    {
    	dns_packet.question->qname[offset] = bytes[12+offset];
    	offset++;
    }
    dns_packet.question->qname[offset] = '\0';

    dns_packet.question->qtype 			  =	((bytes[13+offset] << 8) & 0xFF00     )|
    										((bytes[14+offset] << 0) & 0xFF  	  );
    dns_packet.question->qclass           =	((bytes[15+offset] << 8) & 0xFF00     )|
    ...
    {
    	dns_packet.answer->name[offset-offset0] = bytes[17+offset];
    	offset++;
    }
    dns_packet.answer->name[offset-offset0] = '\0';
     
    dns_packet.answer->resource->type     =	((bytes[18+offset] <<  8) & 0xFF00    )| 
    									  	((bytes[19+offset] <<  0) & 0xFF  	  );
    dns_packet.answer->resource->rclass   =	((bytes[20+offset] <<  8) & 0xFF00 	  )|
    									  	((bytes[21+offset] <<  0) & 0xFF      );
    dns_packet.answer->resource->ttl	  =	((bytes[22+offset] << 24) & 0xFF000000)| 
    								   	  	((bytes[23+offset] << 16) & 0xFF0000  )|
    								   	  	((bytes[24+offset] <<  8) & 0xFF00    )|
    								   	  	((bytes[25+offset] <<  0) & 0xFF      );
    dns_packet.answer->resource->rdlength = ((bytes[26+offset] <<  8) & 0xFF00    )| 
    										((bytes[27+offset] <<  0) & 0xFF      );
    
    for (int i=0; i<dns_packet.answer->resource->rdlength; i++)
    	dns_packet.answer->rdata[i] = bytes[28+offset+i];
}
