...
{   
	...
	while (qname[run_qname] != '\0')
	{
		while (run_label <= qname[run_qname])
			bytes[run_bytes++] = qname[run_qname+run_label++];
		run_qname += run_label;
...
int bytes_to_DNS(DNS_PACKET dns_packet, unsigned char *bytes)
{
	...
	for (int i=0; i<dns_packet.header.ancount; i++)
	{
		...
		memcpy(dns_packet.answer[i].rdata, bytes+28+offset, 
						dns_packet.answer[i].rdlength);
	...
	for (int i=0; i<dns_packet.header.nscount; i++)
	{
		...
		memcpy(dns_packet.authority[i].rdata, bytes+28+offset, 
						dns_packet.authority[i].rdlength);
	...
	for (int i=0; i<dns_packet.header.arcount; i++)
	{
		...
		memcpy(dns_packet.additional[i].rdata, bytes+28+offset, 
						dns_packet.additional[i].rdlength);
...
int main(int argc, char* argv[])
{
	...
	print_bytes(bytes, len_bytes);
	
	DNS_PACKET dns_packets[len_bytes/250+1];

	for (int i=0; i<len_bytes/250+1; i++)
	{
		dns_packets[i].question = malloc(sizeof(dns_packets[i].question));
		dns_packets[i].question->qname = malloc(255);
	}

	int nb_packets = bytes_to_DNSs(dns_packets, msg, len_bytes); 

	printf("Encoded = (%d packets)\n", nb_packets);
	
	unsigned char *dns_packets_binary[len_bytes/250+1];
